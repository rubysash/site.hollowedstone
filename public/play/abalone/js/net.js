// Abalone -- networking layer

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/abalone';
...
export async function sendMove(marbles, direction) {
  const resp = await fetch(`${BASE}/api/move`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ accessCode: _code, token: _token, marbles, direction })
  });
  triggerBurst();
  return resp.json();
}
...
export function saveSession(code, player, token) {
  localStorage.setItem(`abalone_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`abalone_${code}`));
  } catch { return null; }
}
