...
export function setTurnIndicator(isMyTurn) {
  ...
  document.title = isMyTurn ? '>> YOUR TURN << | Abalone' : 'Abalone';
...
export function updatePlayerPanels(state, myPlayer, theme) {
  for (const pid of ['p1', 'p2']) {
    ...
    const marblesEl = panel.querySelector('.marbles-remaining');
    if (marblesEl) marblesEl.textContent = 14 - p.eliminated;
    const scoreEl = panel.querySelector('.score-value');
    if (scoreEl) scoreEl.textContent = `${p.eliminated} / 6`;
  }
}
...
export function addLogEntries(entries, theme) {
  ...
  for (const e of newEntries) {
    ...
    switch (e.action) {
      case 'move':
        div.textContent = `${pName} moves ${e.marbles.join(',')} ${e.type}`;
        break;
      case 'push':
        div.textContent = `${pName} pushes ${e.marbles.join(',')}${e.eliminated ? ' (off the board!)' : ''}`;
        div.classList.add('log-capture');
        break;
      case 'win':
        div.textContent = `${theme.players[e.winner]?.title || e.winner} wins!`;
...
export function showWaiting(accessCode) {
  showOverlay(`
    ...
    <p>Share this code:</p>
    <div class="code-display" onclick="navigator.clipboard.writeText('${accessCode}')" title="Click to copy">${accessCode}</div>
...
export function showGameOver(result, theme) {
  ...
  const reason = result.reason === 'elimination' ? '6 opponent marbles eliminated'
  ...
  showOverlay(`
    <h2>${title}</h2>
    <p>${reason}</p>
    <div style="margin-top:1.5rem;"><a href="/play/abalone/" class="btn">New Game</a></div>
  `);
}
