...
export function addLogEntries(entries, theme) {
  ...
  for (const e of newEntries) {
    ...
    switch (e.action) {
      case 'move':
        div.textContent = `${pName} ${e.from} \u2192 ${e.to}${e.capture ? ' (capture)' : ''}`;
        if (e.capture) div.classList.add('log-capture');
        break;
      case 'win':
...
export function showGameOver(result, theme) {
  ...
  const title = isWin
    ...
    : 'Draw!';
  const reason = result.reason === 'connected' ? 'All pieces connected'
    : result.reason === 'blocked' ? 'Opponent has no legal moves'
    : result.reason === 'move_limit' ? `No captures in ${result.finalScore ? '' : '100'} moves`
  ...
  showOverlay(`
    ...
    <div style="margin-top:1.5rem;">
      <a href="/play/lines-of-action/" class="btn">New Game</a>
    </div>
  `);
}
