// Nine Men's Morris — networking layer
// Polling-based state sync with adaptive rate control

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/nine-mens-morris';
...
export async function sendPlace(node) {
  const resp = await fetch(`${BASE}/api/place`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ accessCode: _code, token: _token, node })
  });
  triggerBurst();
  return resp.json();
}
...
export async function sendRemove(node) {
  const resp = await fetch(`${BASE}/api/remove`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ accessCode: _code, token: _token, node })
  });
...
export function saveSession(code, player, token) {
  localStorage.setItem(`morris_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`morris_${code}`));
  } catch { return null; }
}

export function clearSession(code) {
  localStorage.removeItem(`morris_${code}`);
}
