// Seega -- networking layer

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/seega';
...
export function saveSession(code, player, token) {
  localStorage.setItem(`seega_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`seega_${code}`));
  } catch { return null; }
}
