// Surakarta -- networking layer
let _code = null, _token = null, _player = null;
const BASE = '/play/surakarta';
...
export function initNet(accessCode, token, player) {
  ...
  _lastIdleActivity = Date.now(); startIdleTracking();
}
...
async function doPoll() {
  ...
  try {
    ...
    if (resp.ok) { const state = await resp.json(); if (state && state.logSeq !== undefined) { const hadNew = state.log && state.log.length > 0; _lastLogSeq = state.logSeq; if (hadNew) triggerBurst(); if (_pollCallback) _pollCallback(state); } }
  } catch (e) {}
  scheduleNext();
}
...
function getEffectiveRate() { if (Date.now() < _burstUntil) return POLL_BURST; const idleMs = Date.now() - _lastIdleActivity; if (idleMs > 60000) { _backoffMultiplier = Math.min(_backoffMultiplier * 2, POLL_MAX / _currentRate); return Math.min(_currentRate * _backoffMultiplier, POLL_MAX); } _backoffMultiplier = 1; return _currentRate; }
export function setPollRate(isMyTurn) { _currentRate = isMyTurn ? POLL_MY_TURN : POLL_WAITING; }
export function triggerBurst() { _burstUntil = Date.now() + BURST_DURATION; _backoffMultiplier = 1; clearTimeout(_pollTimer); if (!_stopped) { _nextPollAt = Date.now() + POLL_BURST; _pollTimer = setTimeout(doPoll, POLL_BURST); } }

function startIdleTracking() { const reset = () => { _lastIdleActivity = Date.now(); _backoffMultiplier = 1; }; document.addEventListener('click', reset); document.addEventListener('keydown', reset); document.addEventListener('touchstart', reset); document.addEventListener('visibilitychange', () => { if (!document.hidden) { reset(); triggerBurst(); } }); }

function startCountdown() { clearInterval(_countdownTimer); _countdownTimer = setInterval(() => { const el = document.querySelector('.poll-status'); if (!el) return; const ms = Math.max(0, _nextPollAt - Date.now()); const s = Math.ceil(ms / 1000); let mode = 'active'; if (Date.now() < _burstUntil) mode = 'burst'; else if (_currentRate === POLL_MY_TURN) mode = 'thinking'; else if (_backoffMultiplier > 1) mode = 'idle'; el.textContent = `Sync in ${s}s (${mode})`; }, 250); }

export function saveSession(code, player, token) { localStorage.setItem(`surakarta_${code}`, JSON.stringify({ code, player, token })); }
export function loadSession(code) { try { return JSON.parse(localStorage.getItem(`surakarta_${code}`)); } catch { return null; } }
