// Fanorona — networking layer

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/fanorona';
...
export async function sendMove(from, to, captureType) {
  const resp = await fetch(`${BASE}/api/move`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ accessCode: _code, token: _token, from, to, captureType: captureType || null })
  });
  triggerBurst();
  return resp.json();
}

export async function sendEndChain() {
  const resp = await fetch(`${BASE}/api/endchain`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function saveSession(code, player, token) {
  localStorage.setItem(`fanorona_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`fanorona_${code}`));
  } catch { return null; }
}
