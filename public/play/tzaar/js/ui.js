...
import { ALL_NODES } from './shared/board-data.js';
...
const TYPE_NAMES = { tzaar: 'Tzaar', tzarra: 'Tzarra', tott: 'Tott' };

const WIN_REASONS = {
  type_eliminated: 'Opponent lost a piece type',
  no_capture: 'Opponent cannot capture',
  abandon: 'Opponent left the game'
};
...
export function updatePlayerPanels(state, myPlayer, theme) {
  for (const pid of ['p1', 'p2']) {
    ...
    const capturedEl = panel.querySelector('.captured-count');
    if (capturedEl) capturedEl.textContent = p.captured;

    // Count piece type tops on board for this player
    const typeCounts = { tzaar: 0, tzarra: 0, tott: 0 };
    for (const id of ALL_NODES) {
      const piece = state.board[id];
      if (piece && piece.owner === pid) {
        typeCounts[piece.type]++;
      }
    }

    const tzaarEl = panel.querySelector('.type-tzaar');
    const tzarraEl = panel.querySelector('.type-tzarra');
    const tottEl = panel.querySelector('.type-tott');
    if (tzaarEl) tzaarEl.textContent = typeCounts.tzaar;
    if (tzarraEl) tzarraEl.textContent = typeCounts.tzarra;
    if (tottEl) tottEl.textContent = typeCounts.tott;
  }
}
...
export function addLogEntries(entries, theme) {
  ...
  for (const e of newEntries) {
    ...
    switch (e.action) {
      case 'capture':
        div.textContent = `${pName} ${TYPE_NAMES[e.pieceType] || '?'} captures ${TYPE_NAMES[e.capturedType] || '?'}${e.capturedHeight > 1 ? ` (h${e.capturedHeight})` : ''}`;
        div.classList.add('log-capture');
        break;
      case 'stack':
        div.textContent = `${pName} stacks ${TYPE_NAMES[e.topType] || '?'} (h${e.newHeight})`;
        div.classList.add('log-stack');
        break;
      case 'pass':
        div.textContent = `${pName} passes`;
        break;
      case 'win':
        div.textContent = `${theme.players[e.winner]?.title || e.winner} wins! ${WIN_REASONS[e.reason] || e.reason}`;
...
export function showGameOver(result, theme) {
  ...
  const title = isWin
    ...
    : 'Draw!';
  const reason = WIN_REASONS[result.reason] || result.reason || '';

  showOverlay(`
    <h2>${title}</h2>
    <p>${reason}</p>
    <div style="margin-top:1.5rem;">
      <a href="/play/tzaar/" class="btn">New Game</a>
    </div>
  `);
}
