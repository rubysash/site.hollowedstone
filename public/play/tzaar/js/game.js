// TZAAR -- game controller

import { initNet, pollState, stopPolling, sendMove, sendPass, sendLeave,
...
import { getSelectablePieces, getLegalTargets } from './shared/engine.js';
...
export async function startGame() {
  ...
  if (!code || !token || !player) {
    window.location.href = '/play/tzaar/';
    return;
  }
  ...
  try {
    const resp = await fetch('/play/tzaar/themes/neutral.json');
...
function handlePlayingPhase(state, me, isMyTurn) {
  if (isMyTurn) {
    const actionNum = state.turn.actionNum;
    ...
    if (_selectedNode && state.board[_selectedNode] && state.board[_selectedNode].owner === me) {
      const targets = getLegalTargets(state, me, _selectedNode);
      if (targets.length > 0) {
        clearHighlights();
        highlightSelected(_selectedNode);
        showValidTargets(targets);
        setActionStatus(actionNum);
        return;
      }
    }
...
function setActionStatus(actionNum) {
  if (actionNum === 1) {
    setStatus(_theme.objectives.playing_capture);
  } else {
    setStatus(_theme.objectives.playing_second);
  }
}
...
async function onNodeClick(node) {
  ...
  if (_selectedNode) {
    ...
    // Click a valid target: move (capture or stack)
    const targets = getLegalTargets(_state, me, _selectedNode);
    ...
    if (board[node] && board[node].owner === me) {
      const moves = getLegalTargets(_state, me, node);
      if (moves.length > 0) {
        _selectedNode = node;
        clearHighlights();
...
async function onPassClick() {
  if (!_state) return;
  if (_state.turn.player !== _state.you) return;
  if (_state.turn.actionNum !== 2) return;
