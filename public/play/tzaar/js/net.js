// TZAAR -- networking layer

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/tzaar';
...
export async function sendPass() {
  const resp = await fetch(`${BASE}/api/pass`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function saveSession(code, player, token) {
  localStorage.setItem(`tzaar_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`tzaar_${code}`));
  } catch { return null; }
}
