...
export function setTurnIndicator(isMyTurn) {
  ...
  document.title = isMyTurn ? '>> YOUR TURN << | Tablut' : 'Tablut';
...
export function addLogEntries(entries, theme) {
  ...
  for (const e of newEntries) {
    ...
    switch (e.action) {
      case 'move': {
        const pieceName = e.piece === 'king' ? 'King' : '';
        const capText = e.captured?.length ? ` (captures ${e.captured.join(',')})` : '';
        div.textContent = `${pName} ${pieceName} ${e.from} \u2192 ${e.to}${capText}`;
        if (e.captured?.length) div.classList.add('log-capture');
        break;
      }
      case 'win':
        div.textContent = `${theme.players[e.winner]?.title || e.winner} wins! (${e.reason === 'escape' ? 'King escaped' : e.reason === 'king_captured' ? 'King captured' : e.reason})`;
...
export function showGameOver(result, theme) {
  ...
  const reason = result.reason === 'escape' ? 'The King escaped to a corner'
    : result.reason === 'king_captured' ? 'The King was captured'
    : result.reason === 'stalemate' ? 'No legal moves'
    : result.reason === 'repetition' ? 'Threefold repetition'
    : '';
  showOverlay(`<h2>${title}</h2><p>${reason}</p><div style="margin-top:1.5rem;"><a href="/play/tablut/" class="btn">New Game</a></div>`);
}
...
export function showAbandoned(result, theme) {
  ...
  showOverlay(`<h2>Game Abandoned</h2><p>${result.abandonedBy ? 'Your opponent left.' : 'You forfeited.'}</p><p>${winner} wins by default.</p><div style="margin-top:1.5rem;"><a href="/play/tablut/" class="btn">New Game</a></div>`);
}
