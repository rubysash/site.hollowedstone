...
export function updatePlayerPanels(state, myPlayer, theme) {
  for (const pid of ['p1', 'p2']) {
    ...
    const arrowsEl = panel.querySelector('.arrow-count');
    if (arrowsEl) arrowsEl.textContent = state.arrowCount || 0;
  }
}
...
export function addLogEntries(entries, theme) {
  ...
  for (const e of newEntries) {
    ...
    switch (e.action) {
      case 'move':
        div.textContent = `${pName} ${e.from} \u2192 ${e.to}`;
        break;
      case 'shoot':
        div.textContent = `${pName} arrow \u2192 ${e.target}`;
        div.classList.add('log-shoot');
        break;
      case 'win':
        div.textContent = `${theme.players[e.winner]?.title || e.winner} wins! (No legal moves)`;
...
export function showGameOver(result, theme) {
  ...
  const title = isWin
    ...
    : 'Draw!';
  const reason = result.reason === 'blocked' ? 'No legal moves'
  ...
  showOverlay(`
    ...
    <div style="margin-top:1.5rem;">
      <a href="/play/amazons/" class="btn">New Game</a>
    </div>
  `);
}
