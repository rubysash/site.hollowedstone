// Amazons -- networking layer

let _code = null;
let _token = null;
let _player = null;

const BASE = '/play/amazons';
...
export async function sendShoot(target) {
  const resp = await fetch(`${BASE}/api/shoot`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ accessCode: _code, token: _token, target })
  });
  triggerBurst();
  return resp.json();
}
...
export function saveSession(code, player, token) {
  localStorage.setItem(`amazons_${code}`, JSON.stringify({ code, player, token }));
}

export function loadSession(code) {
  try {
    return JSON.parse(localStorage.getItem(`amazons_${code}`));
  } catch { return null; }
}
